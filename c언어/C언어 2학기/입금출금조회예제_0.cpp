...
void commains(int money, char retstr[50]);
void main()
{
	int sel;
	while (1)
...
void FDayToDay()
{
	...
	fclose(rfp1);
	fclose(rfp2);
	char retstr1[50], retstr2[50], retstr3[50];
	printf("---------------------------------------\n");
	commains(intot, retstr1);
	commains(outtot, retstr2);
	commains(intot-outtot, retstr3);
	printf("\t%s  \t  %s  \t  %s\n",  retstr1, retstr2, retstr3);
	printf("---------------------------------------\n");
...
void commains(int money, char retstr[50])
{
	int intLength;
	int i, cnt = 1, pos = 0;
	char strBuffer[50]; // 문자열을 저장할 버퍼
	char str[50] = " ";
	char str1[50] = " ";
	itoa(money, strBuffer, 10);

	intLength = strlen(strBuffer);
